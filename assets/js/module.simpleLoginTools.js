 ...
 */
  .service('waitForAuth', function($rootScope, $q, $timeout) {
    ...
    // TODO: Change to parse login.
    
    subs.push($rootScope.$on('loginService:login', fn));
    subs.push($rootScope.$on('loginService:logout', fn));
    subs.push($rootScope.$on('loginService:error', fn));
    return def.promise;
  })
 ...
 */
  .directive('ngShowAuth', function ($rootScope) {
    ...
    $rootScope.$on('loginService:login',  function() { loginState = 'login'; });
    $rootScope.$on('loginService:logout', function() { loginState = 'logout'; });
    $rootScope.$on('loginService:error',  function() { loginState = 'error'; });
    ...
    return {
      ...
      link: function(scope, el, attr) {
        ...
        fn();
        $rootScope.$on('loginService:login',  fn);
        $rootScope.$on('loginService:logout', fn);
        $rootScope.$on('loginService:error',  fn);
      }
    };
  });